...
describe('plugin host registry', () => {
  ...
  test('output generators are registered by plugin-qualified key and reject duplicates', () => {
    ...
    expect(getAvailableOutputGenerators().map((generator) => generator.key)).toEqual(['dev.hvy.resume.skill-description']);
    expect(getOutputGenerator('dev.hvy.resume.skill-description')?.label).toBe('Generate description');
    ...
    })).toThrow('Duplicate output generator key "dev.hvy.resume.skill-description".');
  });
...
describe('progress-bar plugin block round-trip', () => {
  test('preserves pluginConfig and text body across serialize/deserialize', () => {
    const input = `---\nhvy_version: 1.0\n---\n\n#! Status\n\n<!--hvy:plugin {"plugin":"dev.hvy.progress-bar","pluginConfig":{"min":0,"max":100,"value":42,"color":"#3b82f6"}}-->\n \`\${value}%\`\n`;
    ...
    expect(reserialized).toContain('"plugin":"dev.hvy.progress-bar"');
...
describe('scripting plugin block metadata', () => {
  test('preserves scripting plugin version in pluginConfig across serialize/deserialize', () => {
    const input = `---\nhvy_version: 1.0\n---\n\n#! Script\n\n<!--hvy:plugin {"plugin":"dev.hvy.scripting","pluginConfig":{"version":"${SCRIPTING_PLUGIN_VERSION}"}}-->\nprint("hello")\n`;
    const doc = deserializeDocument(input, '.hvy');
    const block = doc.sections[0]?.blocks.find((b) => b.schema.component === 'plugin');
    expect(block).toBeDefined();
    ...
    const reserialized = serializeDocument(doc);
    expect(reserialized).toContain('"plugin":"dev.hvy.scripting"');
...
describe('form plugin block round-trip', () => {
  test('preserves form plugin config and YAML body across serialize/deserialize', () => {
    const input = `---\nhvy_version: 1.0\n---\n\n#! Order\n\n<!--hvy:plugin {"plugin":"dev.hvy.form","pluginConfig":{"version":"0.1","submitScript":"submit_form"}}-->\n fields:\n   - label: Food\n     type: select\n     options:\n       - Apple\n       - label: Soup\n         value: soup\n scripts:\n   submit_form: |\n     doc.header.set("submitted", doc.form.get_values())\n`;
    const doc = deserializeDocument(input, '.hvy');
    const block = doc.sections[0]?.blocks.find((b) => b.schema.component === 'plugin');
    expect(block).toBeDefined();
    ...
    const reserialized = serializeDocument(doc);
    expect(reserialized).toContain('"plugin":"dev.hvy.form"');
...
describe('plugin block selector swap', () => {
  test('swapping plugin id resets pluginConfig and text', () => {
    const input = `---\nhvy_version: 1.0\n---\n\n#! Status\n\n<!--hvy:plugin {"plugin":"dev.hvy.db-table","pluginConfig":{"source":"with-file","table":"work"}}-->\n SELECT * FROM work\n`;
    const doc = deserializeDocument(input, '.hvy');
    const block = doc.sections[0]?.blocks.find((b) => b.schema.component === 'plugin');
    expect(block).toBeDefined();
