...
export function buildHeaderEditFormatInstructions(options?: { planActive?: boolean }): string {
  ...
  return [
    ...
    'Use the header path for document-level metadata, theme colors, component defaults, section defaults, text line styles, template schema, plugins, and component template/section definitions.',
    ...
    'Use `grep_header` to search the YAML header with a regex pattern before viewing or patching a specific component template or section template definition.',
    ...
    'Use `patch_header` to edit metadata, component template definitions, or section template definitions after you have enough numbered header context. Patch the YAML header content without `---` delimiters.',
    ...
    ...(planActive ? [] : ['{"tool":"plan","steps":["Find the component template definition","Patch the YAML","Verify the header"],"reason":"optional"}']),
    '{"tool":"mark_step_done","step":1,"summary":"Found the component template definition.","reason":"optional"}',
...
export function buildInitialHeaderEditPrompt(request: string): string {
  return [
    ...
    'Use this path for document metadata and template definitions: title, reader settings, theme, defaults, template schema, plugins, `component_defs`, and `section_defs`.',
