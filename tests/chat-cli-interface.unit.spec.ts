...
test('chat cli runs commands against the document filesystem and persists mutations', async () => {
  ...
  expect((await cli.run('hvy insert -1 section /body chores "Chores"')).mutated).toBe(true);
  const addTextResult = await cli.run('hvy insert -1 text /body/chores note "Weekly chore plan"');
