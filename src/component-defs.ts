...
export function getComponentOptions(): string[] {
  const builtins = ['text', 'image', 'button', 'expandable', 'container', 'component-list', 'grid', 'plugin', 'xref-card'];
  if (areTablesEnabled()) {
...
export function isBuiltinComponentName(componentName: string): boolean {
  return ['text', 'image', 'button', 'expandable', 'table', 'container', 'component-list', 'grid', 'plugin', 'xref-card'].includes(componentName);
}
