...
test('resume skill description generator uses only provided skill value', async () => {
  const generator = resumeOutputGeneratorsPlugin.outputGenerators?.find((item) => item.key === 'dev.hvy.resume.skill-description');
...
test('resume tool description generator uses only provided tool value', async () => {
  const generator = resumeOutputGeneratorsPlugin.outputGenerators?.find((item) => item.key === 'dev.hvy.resume.tool-description');
