...
test('getDocumentDbTableNames finds plugin blocks nested inside containers', async () => {
  ...
  const nestedDoc = deserializeDocument(
    `---\nhvy_version: 0.1\n---\n\n<!--hvy: {"id":"root"}-->\n#! Root\n\n <!--hvy:text {}-->\n  Intro\n\n <!--hvy:container {"id":"pipeline"}-->\n  <!--hvy:plugin {"plugin":"hvy.db-table","pluginConfig":{"source":"with-file","table":"job_applications"}}-->\n`,
    '.hvy'
  );
...
test('buildDbTableEditContext embeds schema, sample rows, and the component fragment', () => {
  const context = buildDbTableEditContext({
    ...
    fragment: '<!--hvy:plugin {"plugin":"hvy.db-table","pluginConfig":{"table":"work_items"}}-->',
...
test('requestAiDbTableEdit returns a replacement block when the model emits edit_fragment', async () => {
  ...
  const replacementFragment =
    '<!--hvy:plugin {"plugin":"hvy.db-table","pluginConfig":{"source":"with-file","table":"work_items","query":"SELECT * FROM work_items WHERE status != \'Rejected\'"}}-->';
  ...
  expect(result.block.schema.plugin).toBe('hvy.db-table');
...
test('requestAiDbTableEdit rejects blocks without a configured table name', async () => {
  ...
  const doc = deserializeDocument(
    `---\nhvy_version: 0.1\n---\n\n#! S\n\n <!--hvy:plugin {"plugin":"hvy.db-table","pluginConfig":{"source":"with-file"}}-->\n`,
    '.hvy'
  );
