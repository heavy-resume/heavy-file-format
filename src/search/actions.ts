...
import { navigateToEditorTarget, revealEditorTargetInState, findBlockPath } from '../editor-target-navigation';
...
export function selectSearchResult(app: HTMLElement, resultId: string): void {
  ...
  const requiresEditorReveal = state.currentView === 'editor' && revealEditorTargetInState(result);
  const requiresFullRender = isEnteringQuickSearch || requiresEditorReveal;
  const navigate = () => {
    if (state.currentView === 'editor') {
      navigateToEditorTarget(result, app);
      return;
    }
