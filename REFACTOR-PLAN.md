...
- Parser fix: `expandable:stub` / `expandable:content` with only `{lock}` (or empty) is the part-header form and sets part lock without opening a block; all other payload shapes open a block and the `lock` field is interpreted as the block's own lock, not the part's. Previously these were conflated.
- Examples/resume.thvy: already consistent with this phase (uses new names; slot payloads still carry `component` until the restructuring phase).
- Examples/resume.hvy: ran `scripts/migrate-slots.py` to split slot-with-component markers into empty slot + nested child directive (indent-bump tracked across splits; section headers/directives/fences don't get bumped). File is now in target format and serves as a test case to drive parser/serializer updates. Script retained at `scripts/migrate-slots.py` for running on other examples if needed.
- Examples/resume.hvy: removed 16 spurious empty `<!--hvy:expandable {}-->` wrappers previously inserted inside custom-component (`skill-record`, `history-record`, `project-record`, `education-record`) list slots. The custom component's baseType is already `expandable`, so those wrappers were creating an anonymous second expandable and shadowing the record body. With them gone, `expandable:stub` / `expandable:content` markers at indent+1 correctly attach to the custom-component frame.
