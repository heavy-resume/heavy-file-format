...
- `export_prompt_templates`: optional array of reusable prompt templates for AI-assisted export planning. Each entry has `id`, `label`, optional `description`, `prompt`, and optional `variables`. Prompt variables use reusable template token syntax such as `{% target_context | block %}` or `{% audience_note | text %}`. `variables` maps variable names to UI hints: `label`, `placeholder`, `helpText`, `required`, and `type` (`text` or `block`). Export planners MAY render one of these templates with user-provided values to produce a generic export prompt, then derive a content view and export strategy. The metadata MUST NOT imply a domain-specific API; pasted job descriptions, audience notes, grant criteria, project briefs, and similar inputs are all just template values. Readers that do not use AI export planning SHOULD preserve and ignore it.
