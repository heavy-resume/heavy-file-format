...
test('requestDocumentEditChatTurn runs the CLI edit loop for document chat', async () => {
  ...
  expect(requestProxyCompletionMock.mock.calls[0]?.[0]).toEqual(
    expect.objectContaining({
      ...
      systemInstructions: expect.stringContaining('Valid commands (in order of preference):\nCommands: hvy, nl, rg, find, sed, printf, echo, cat, ls, pwd, cd, cp, mv, rm, grep, sort, uniq, wc, tr, xargs, head, tail, true. Ask: use ask_user tool. Finish: use finish_task tool.'),
    })
  );
...
<!--hvy:plugin {"id":"contact-form","plugin":"hvy.form","pluginConfig":{"version":"0.1","submitLabel":"Send"}}-->
fields:
  - label: Message
    type: textarea
`, '.hvy');
  ...
  expect(nextPrompt).toContain('Plugin id: hvy.form (form).');
...
`, '.hvy');
  ...
  expect(result.error).toBeNull();
  const nextPrompt = requestProxyCompletionMock.mock.calls[1]?.[0]?.messages.at(-1)?.content ?? '';
  expect(nextPrompt).toContain('Plugin id: hvy.scripting (scripting).');
